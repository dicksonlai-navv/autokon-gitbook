---
description: Ikuti langkah-langkah sederhana ini untuk mencatat perbaikan cacat di lokasi dengan WhatsApp.
icon: gambar
---

# Cara Mencatat Perbaikan Cacat melalui WhatsApp

## Langkah 0: Cetak Laporan Inspeksi untuk Referensi

Masuk ke WebApp AutoKon dan navigasi ke menu **Manajemen Cacat > Ikhtisar** dari sidebar.

Pilih **unit(-unit)** yang relevan dan klik **"Ekspor ke PDF"** untuk mengunduh dan mencetak laporan inspeksi sebagai referensi selama perbaikan cacat.

{% hint style="info" %}
Cetak laporan inspeksi sebelumnya untuk dengan mudah mencocokkan setiap cacat dengan ID Cacatnya.\
Ini juga membantu Anda mengidentifikasi dengan tepat cacat yang memerlukan perbaikan di lokasi.
{% endhint %}

<figure><img src="../.gitbook/assets/Untitled design (3).png" alt=""><figcaption></figcaption></figure>

## Langkah 1: Buka & Aktifkan Bot

Buka obrolan WhatsApp Anda dengan AutoKon dan kirim **"Hi"** untuk mengaktifkan bot.

<figure><img src="../.gitbook/assets/AutoKon - How To Mockup (6).png" alt="" width="375"><figcaption></figcaption></figure>

## Langkah 2: Pilih Unit

Pilih **ID Unit** di mana Anda ingin mencatat perbaikan cacat.

<figure><img src="../.gitbook/assets/17.png" alt="" width="375"><figcaption></figcaption></figure>

## Langkah 3: Pilih ID Cacat&#x20;

Pilih **ID Cacat** di mana Anda ingin mencatat perbaikan cacat.

{% hint style="info" %}
Tip: Cetak laporan inspeksi sebelumnya untuk dengan mudah mencocokkan setiap cacat dengan ID Cacatnya.\
Ini juga membantu Anda mengidentifikasi dengan tepat cacat yang memerlukan perbaikan di lokasi.
{% endhint %}

<figure><img src="../.gitbook/assets/18 (1).png" alt="" width="375"><figcaption></figcaption></figure>

## Langkah 4: Ambil Gambar Perbaikan Cacat

Ambil **foto yang jelas tentang perbaikan cacat** dan kirim di obrolan.

<figure><img src="../.gitbook/assets/19.png" alt="" width="375"><figcaption></figcaption></figure>

## Langkah Terakhir: Lanjutkan atau Akhiri Sesi

Pilih untuk **mencatat cacat lain**, **mengganti unit**, atau **mengakhiri sesi**.

<figure><img src="../.gitbook/assets/20 (1).png" alt="" width="375"><figcaption></figcaption></figure>

✅ **Itu saja**! Perbaikan cacat berhasil dicatat dan akan muncul di dasbor Anda secara real-time. Pengembang real estat Anda juga akan diberitahu tentang pengajuan Anda.